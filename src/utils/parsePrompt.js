...
/**
 * Parse a customer prompt to extract customer name and steel items
...
export function parsePrompt(userPrompt) {
  ...
  try {
    // Extract customer name using pattern "To CUSTOMER_NAME,"
    const customerMatch = userPrompt.match(/To\s+([^,\n]+),/i);
    if (customerMatch) {
      result.customer = customerMatch[1].trim();
    } else {
      // Try alternative patterns
      const altCustomerMatch = userPrompt.match(/customer[:\s]+([^\n,]+)/i) ||
                              userPrompt.match(/for\s+([^,\n]+)/i) ||
                              userPrompt.match(/dear\s+([^,\n]+)/i);
      if (altCustomerMatch) {
    ...
    const transportMatch = userPrompt.match(/transport\s+([^,\n\.]+)/i);
    if (transportMatch) {
      result.transport = transportMatch[1].trim();
    }

    const loadingMatch = userPrompt.match(/loading\s+([^,\n\.]+)/i);
    if (loadingMatch) {
      result.loading = loadingMatch[1].trim();
    }

    // Step 1: Extract items with quantities (without rates initially)
    const itemsWithQty = extractItemsWithQuantities(userPrompt);
    
    // Step 2: Extract rates separately
    const ratesData = extractRates(userPrompt);
    
    // Step 3: Match items with their rates
    const matchedItems = matchItemsWithRates(itemsWithQty, ratesData);
    
    result.items = matchedItems;

    // If no items found with our regex approach, log for debugging
    if (result.items.length === 0) {
      console.log('No items found with regex, raw prompt:', userPrompt);
